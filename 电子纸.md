...
- 具有旧版本的软件，设备可能会冻结或变得难以使用，直到索引完成
- 具有更新版本的软件，索引过程更温和，但需要更长的时间
...
无论什么原因，你可能想跳过这一步，使用你的电子阅读器，而不会因为这个功能而失去能源。或者你根本不关心这个搜索功能，只想永远禁用它。



简单的解决方案是将您的设备插入计算机上，将目录“system/Search Indexes”删除（您可以猜到这个目录包含索引数据），然后创建一个名为Search Indexes的空文件，这一步很重要，如果您只是删除这个文件夹，它将被重新创建。然而，这个过程只是使得索引器进入触发错误的循环中。索引任务本身并没有被停止（在主页搜索框搜索`;dm`然后查看转存的消息文档），但总之，比正确的索引过程要省电得多。



索引任务由可执行文件`/etc/upstart/framework`中的`DENABLE_SEARCH_INDEXING_THREAD`标志位控制，如果你的Kindle越狱了，可以通过SSH连接上Kindle，通过shell在此文件中搜索该标志位并将其值修改为`false`来彻底禁用索引任务。



参考

- https://www.mobileread.com/forums/showthread.php?t=279955
- https://www.mobileread.com/forums/showthread.php?t=116981&page=4
