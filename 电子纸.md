...
> **Carta迭代的提升并不大**
>
> Carta1000：反应速度450ms、对比度15:1，那么
>
> - Carta 1100/1200：反应速度360ms、对比度17.25:1
> - Carta 1300：反应速度328.5ms、对比度18:1
>
...
> 参考
>
> - [E Ink 元太科技 电子墨水品牌](https://cn.eink.com/brand?bookmark=Carta)
> - [E Ink display](https://wiki.mobileread.com/wiki/E_Ink_display)
...
DRM（Digital Rights Management，数字版权管理）通常旨对特定文件的操作实施限制，保护电子书（或音乐和其他电子媒体）的作者和出版商免受盗版和其他非法活动的侵害。



MOBI（`.mobi`）是MobiPocket开发的电子书格式，Mobipocket软件包是免费的，包括各种发布和阅读工具。亚马逊2005年收购了Mobipocket一直运行到2016年10月关闭了Mobipocket网站和服务器。



Kindle File Format（Kindle文件格式）是亚马逊创建的私有电子书格式

- 2007年，最初版本：基于MobiPocket，具有文件扩展名`.azw`
- 2011 年末，版本8（Kindle Format 8，KF8），也称为 AZW3 文件格式：引入HTML5和CSS3功能，具有文件扩展名`.azw3`
- 2015 年 8 月，版本10（Kindle Format 10， KFX）：引入了新的排版和布局引擎，具有连字符、字距调整和连字功能，具有文件扩展名`.kfx`
...
搜索框执行`;dm`命令，在生成的`all_system_logs_`文件中搜索`battinfo`，你会看到类似以下的结果

```bash
batt_type = PRIMARY, capav=89%, mAh=909mAh, volt=4202000mV, current=-105228mA, temp=69F, lmd=0mAh, cycl=0
```

使用以下公式获取电池健康程度百分比：

- 最大实际容量：`(mAh / capav) * 100`
- 电池健康百分比：`(最大实际容量/ 标称容量) * 100`

要找到你的Kindle型号的标称容量，请参阅[这篇Wikipedia文章](https://en.wikipedia.org/wiki/Amazon_Kindle#Specifications)，查看Battery栏。

> [!TIP]
>
> 参考
>
> - [How to determine Kindle's battery health](https://www.reddit.com/r/kindle/comments/ucfva0/how_to_determine_kindles_battery_health/)

## 海淘Kindle

[手把手教你海淘美亚 Kindle 电子书：注册、购买、转运全流程详解](https://uuzi.net/how-to-buy-kindle-books-from-amazon-usa/)

> [!TIP]
>
> 亚马逊账号上添加支付方式（支持**银联**、VISA等信用卡和借记卡）

> [!IMPORTANT]
>
> 下单就做好不能退货的准备！！！亚马逊退货标签不再显示Tracking ID，向客服查询特别麻烦。



**Prime Day（亚马逊每年7月举行的Prime会员日） 和 黑色星期五（美国感恩节后第二天，即十一月第四个星期五）是美国亚马逊促销力度最大的时间**
...
> [!IMPORTANT]
>
> **只有将派送地点改为中国地址，客户服务（Customer Service）才会有邮件联系选项，页面语言才会有中文简体选项**。
>
> 当在线聊天服务可用，但点击后窗口展现错误时说明你的账号可能被禁止使用在线聊天服务，可以选择邮件联系请求解除封禁。
...
- 系统内置字体：`/usr/java/lib/fonts/`
- 系统下载字体：`/var/local/font/mnt`
...
- CPU：MT8110 Bellatrix device，双核，ARMv7 Processor rev 4 (v7l)
- RAM：1GB
- 屏幕
  - 分辨率：6寸，1072 x 1448，300PPI
  - eink waveform版本：520_VSH029_HW2F01_ED060KHFC2_TC
...
- 尺寸：长157.8 mm、宽108.6 mm、厚8.0 mm
- 重量：158g
...
- 尺寸：长 176.7 mm，宽127.6mm，厚7.8 mm
- 重量：标准版211g，签名版214g



价格：标准版`$159.99`，签名版`$199.99`

- 2024黑色星期五：标准版`$129.99`，签名版`$154.99`
- 2025情人节：标准版`$134.99`，签名版`$199.99`
...
手动更新脚本：`/extensions/koreader/bin/koreader-ext.sh`
...
更新只是将koreader包文件解压到`/mnt/us`，因此如果更新过程中未发生错误，只需等待一小会即可。

> [!TIP]
>
> 更新脚本执行时左下角会一直显示`Try to update KOReader...`信息而且似乎什么事也没有发生，查看KUAL日志似乎是因为无法刷新屏幕，但这并不影响。
>
> ```bash
> [FBInk] Unidentified Kindle device 3KM (0xE75) [G093KM]!
...
> [!TIP]
>
> 如果发现未更新成功，可能是由于KOReader SSH进程运行的原因，查看KUAL日志发现
>
> ```bash
> unzip: can't open 'koreader/dropbear': Text file busy
> ```
>
> 在KOReader中关闭SSH服务器即可，如果仍未成功，检查是否有存在未正常关闭的`dropbear`进程并结束它。

## 字体

`koreader/frontend/fontlist.lua`中`FontList:_readList`方法是读取字体的逻辑，以下是kindle字体黑名单，值为true的kindle字体将不会展示在KOReader的字体列表中。
...
1. 将相应文件放入`koreader/fonts`下，并将相应值修改成false
2. 将文件名修改为非上述列表名称
...
- 4
  ...
  - Calibre -> 无线网络设置 -> ☑️启用无线客户端（使用时再打开）
  - 进度同步 -> 注册/登录

## 保存设置

将**底部菜单设置 + 字体类型**保存为配置文件：顶部菜单4->配置文件->使用当前书籍设置新建

## 备份设置

在极少数情况下，如果您想要备份和恢复 KOReader 的所有设置，您可以备份 KOReader 安装目录下的这些文件夹和文件。然后，您可以在全新安装后覆盖它们以恢复您的设置：
...
- **文件**
  ...
  -  **`data/dict/`** 和 **`/data/tessdata/`** 目录下的文件 **(如果它们存在)**


> [!TIP]
>
> 您的个人书籍设置和高亮保存在和您的书籍同一目录中，文件跟书籍同名。如果您愿意，可以分别备份这些文件夹。

# Calibre

## 概述

calibre 是一个电子书库管理器。它能够查看、转换、分类主流的电子书格式，同时还能够连接多种电子书阅读器。
